module ApplicationProcessor
  ...
  def to_context(ruleset, applicant)
    ...
    input.merge!('State' => @state,
                 ...
                 'Tax Returns' => @tax_returns).slice(*ruleset.class.inputs.keys)
  ...
  def determine_household(person)
    ...
    MedicaidHousehold.new(nil, med_household_members, nil, nil, household_size)
  end
  ...
  def calculate_household_size(person, med_household_members)
    ...
    if @config['Count Unborn Children for Household'] == '01'
      return med_household_members.count +
             med_household_members.inject(0) do |sum, p|
               sum +
  ...
  def calculate_household_income(income_people)
    income_people.select(&:income).map do |p|
      p.income[:incomes].inject(0) { |sum, (_name, amt)| sum + amt } -
        p.income[:deductions].inject(0) { |sum, (_name, amt)| sum + amt }
    end.sum
  end
end
