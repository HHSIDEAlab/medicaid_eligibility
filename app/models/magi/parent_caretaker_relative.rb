...
module MAGI
  class ParentCaretakerRelative < Ruleset
    ...
    calculated 'Applicant Child List' do
      ...
      children = v('Applicant Relationships').select do |rel|
        [:child, :stepchild].include?(rel.relationship_type)
      end.map(&:person)
      ...
      if v('Applicant Age') >= c('Child Age Threshold')
        responsible_children = v('Applicant Relationships').select do |rel|
      ...
      for child in children
        child_input = {
          ...
          'Child Parents'      => child.relationships.select { |r| r.relationship_type == :parent }.map(&:person),
    ...
    rule 'Child of Caretaker meets all criteria' do
      o['Qualified Children List'] = v('Applicant Child List').select do |child|
