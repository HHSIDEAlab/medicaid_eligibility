module ApplicationParser
  ...
  def read_configs!
    ...
    @config = if config[@state]
                config[:default].merge(config[@state])
              else
                config[:default]
              end
  ...
  def read_json!
    ...
    for json_person in @json_application['People']
      ...
      unless begin
                ...
                false
              end # person_id.is_a?(Integer) && person_id >= 1 && person_id <= 100
      ...
      @applicants << person if is_applicant
      @people << person
    end

    unless @people.map(&:person_id).length == @people.map(&:person_id).uniq.length
    ...
    for json_return in @json_application['Tax Returns']
      ...
      filers = json_return['Filers'].map do |jf|
        @people.find { |p| p.person_id == jf['Person ID'] }
      end

      dependents = json_return['Dependents'].map do |jd|
  ...
  def read_xml!
    ...
    for xml_person in xml_people
      ...
      xml_app = get_nodes('/exch:AccountTransferRequest/hix-ee:InsuranceApplication/hix-ee:InsuranceApplicant').find do |x_app|
        x_app.at_xpath('hix-core:RoleOfPersonReference').attribute('ref').value == person_id
      end
      if xml_app
    ...
    # get relationships
    for person in @people
      xml_person = get_nodes('/exch:AccountTransferRequest/hix-core:Person').find do |x_person|
        x_person.attribute('id').value == person.person_id
      end
    ...
    for xml_return in xml_tax_returns
      ...
      filers = xml_filers.select { |xf| xf }.map do |xf|
        @people.find { |p| p.person_id == get_node('hix-core:RoleOfPersonReference', xf).attribute('ref').value }
      end

      dependents = get_nodes('hix-ee:TaxHousehold/hix-ee:PrimaryTaxFiler/RoleOfPersonReference', xml_return).map do |node|
  ...
  def get_xml_variable(node, input, attributes)
    get_variable(nil, input, attributes) unless node
