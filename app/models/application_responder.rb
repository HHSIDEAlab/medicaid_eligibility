module ApplicationResponder
  def to_json(_options = {})
    return JSON.pretty_generate('Error' => @error.message) unless @error.nil?
    ...
    for app in @applicants
      ...
      app_json['Medicaid Household']['People'] = app.medicaid_household.people.map(&:person_id)
  ...
  def to_xml(_options = {})
    nokogiri_xml = Nokogiri::XML::Builder.new(encoding: 'UTF-8') do |xml|
      xml.send('exch:AccountTransferRequest', Hash[XML_NAMESPACES.map { |k, v| ["xmlns:#{k}", v] }]) do
        xml.send('ext:TransferHeader') do
          xml.send('ext:TransferActivity') do
            xml.send('nc:ActivityIdentification') do
              # TODO
            end
            xml.send('nc:ActivityDate') do
        ...
        xml.send('hix_core:Sender') do
          # TODO
        end
        xml.send('hix_core:Receiver') do
          # TODO
        end
        xml.send('hix-ee:InsuranceApplication') do
          xml.send('hix-core:ApplicationCreation') do
            # TODO
          end
          xml.send('hix-core:ApplicationSubmission') do
            # TODO
          end
          @applicants.each do |applicant|
            xml.send('hix-ee:InsuranceApplicant', 's:id' => applicant.applicant_id) do
              xml.send('hix-ee:MedicaidMAGIEligibility') do
                ApplicationVariables::DETERMINATIONS.select { |det| det[:eligibility] == :MAGI }.each do |determination|
                  det_name = determination[:name]
                  xml.send("hix-ee:MedicaidMAGI#{det_name.gsub(/ +/, '')}EligibilityBasis") do
                    build_determinations(xml, det_name, applicant)
                    if det_name == 'Parent Caretaker Category'
                      xml.send('ChildrenEligibilityBasis') do
                        applicant.outputs['Children List'].each do |child|
                          xml.send('Child', 's:ref' => child['Person ID']) do
                            ApplicationVariables::CHILD_OUTPUTS.each do |output|
                              xml.send(output[:name].gsub(/ +/, '')) do
        ...
        if return_application?
          @people.each do |person|
            xml.send('hix-core:Person', 's:id' => person.person_id) do
            end
          end
          @physical_households.each do |household|
            xml.send('ext:PhysicalHousehold') do
  ...
  def build_determinations(xml, det_name, applicant)
    ...
    xml.send('hix-ee:EligibilityBasisDetermination') do
      xml.send('nc:ActivityDate') do
        xml.send('nc:DateTime', applicant.outputs["#{det_name} Determination Date"].strftime('%Y-%m-%d'))
      end
    end
