...
module Medicaid::Eligibility::Category
  class ParentCaretakerRelative < Ruleset
    ...
    def calculate_child_age(person_birth_date)
      (current_date - person_birth_date)/365.25
    end
    ...
    Business Rule Logic:  For each record in the Applicant Child List determine if child meets four criteria
    1.  Dependent Child Age Logic – Child under dependent age 
      When Dependent Child Age < Dependent Age Threshold
    Then Child of Caretaker Dependent Age Indicator= Y and Child of Caretaker Dependent Age Determination Date = current date and Child of Caretaker Dependent Age Ineligibility Reason = 999
    Continue to Deprivation logic below with the current child record
    2.  Dependent Child Age Logic – Child older than dependent age
      When Dependent Child Age > Dependent Age Threshold
    Then Child of Caretaker Dependent Age Indicator= N and Child of Caretaker Dependent Age Determination Date = current date and Child of Caretaker Dependent Age Ineligibility Reason = 147
    Continue to Deprivation logic below with the current child record
    3.  Dependent Child Age Logic – Child equal to dependent age and a student  When Option Dependent Student = Y  and  Dependent Child Age = Dependent Age Threshold and Student Indicator = Y
    Then Child of Caretaker Dependent Age Indicator= Y and Child of Caretaker Dependent Age Determination Date = current date and Child of Caretaker Dependent Age Ineligibility Reason = 999
